...
pages.page_classrooms = () => {
    ...
    // classLinks.forEach((linkElement) => { linkElement.addEventListener("click",
    // (event) => { event.preventDefault();         const href =
    // linkElement.getAttribute("href");         window.location.href = href; });
...
pages.page_teacher_classwork = () => {
    ...
    // topicsSelectBox.innerHTML += `<option value="null">No topic</option>`; // get
    // the topics fo the class and display them try {     let getTopics = async() =>
    // {         const data = new FormData();         data.append("class_id",
    // classID);         let response = await pages.postAPI(pages.base_url +
    // "fetch_topics.php", data);         response             .data .map((item) =>
    // {                 topicsSelectBox.innerHTML += `<option
    // value="${item.topic_id}">${item.topic_name}</option>`;             });     };
    //     getTopics(); } catch (error) {     console.log(error); } assignment
    // sidebar
    ...
    // let dueDate = dateInput.value; const currentDate = new Date(); const month =
    // currentDate.getMonth() + 1; const day = currentDate.getDate(); const hours =
    // currentDate.getHours(); const minutes = currentDate.getMinutes(); const
    // seconds = currentDate.getSeconds(); let date =
